...
const OvVideoMode OvVideoCapture::OV_MODE_640_480_30 = { 640, 480, 30, 3 };
const OvVideoMode OvVideoCapture::OV_MODE_1920_1080_15 = { 1920, 1080, 15, 2 };
const OvVideoMode OvVideoCapture::OV_MODE_2592_1944_15 = { 2592, 1944, 15, 5 };
...
/*static void to_gray(const unsigned char* input, unsigned char* output, int length) {
	...
	std::ofstream ofs("rawdata.txt", std::ofstream::out);
	...
	std::cout << "to_gray(): length:" << length << std::endl; 
}*/
...
bool OvVideoCapture::open() {
	//ipu_input_format_ = { mode_.width, mode_.height, 16, V4L2_PIX_FMT_SBGGR8 };
...
bool OvVideoCapture::retrieve(cv::Mat& image) {
	...
	cv::Mat bayer8BitMat(mode_.height, mode_.width, CV_8UC1, buffer_);
	cv::cvtColor(bayer8BitMat, image, CV_BayerBG2RGB, 3);

	//cv::Mat bayer16BitMat(mode_.height, mode_.width, CV_16UC1, buffer_);
	...
	//std::cout << "convert returned " << std::endl;
...
bool OvVideoCapture::open_internal() {
	...
	fszenum.index = mode_.capture_mode;
	fszenum.pixel_format = DefaultFormat; //V4L2_PIX_FMT_SBGGR8;

	if (ioctl(fd_, VIDIOC_ENUM_FRAMESIZES, &fszenum) < 0) {
                close(fd_);
                return false;
        }
	...
	fmt.fmt.pix.sizeimage = 0;//frame_size_;
	fmt.fmt.pix.bytesperline = 0;//mode_.width * DefaultFormatChannels;
